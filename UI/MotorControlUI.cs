...
public class MotorControlUI
{
        ...
        SynchronizedDualAxisConfig config)
    {
        ...
        _incrementButton = Button.NewWithLabel("▲");
        _incrementButton.SetSizeRequest(50, 40);
        _incrementButton.OnClicked += OnIncrementSpeedClicked;

        _decrementButton = Button.NewWithLabel("▼");
        _decrementButton.SetSizeRequest(50, 40);
        _decrementButton.OnClicked += OnDecrementSpeedClicked;

        fineAdjustBox.Append(_incrementButton);
        fineAdjustBox.Append(_decrementButton);
    ...
    private async void OnMinLimitButtonClicked(Button sender, EventArgs args)
    {
        try
        {
            ...
            // Only reset position if we successfully reached the min limit (not cancelled)
            if (_motorController.IsMinLimitSwitchTriggered)
            {
        ...
        {
            // Movement was cancelled - don't reset position
        }
        catch (Exception ex)
        {
    ...
    private async void OnStopButtonClicked(Button sender, EventArgs args)
    {
        try
        {
            // Don't cancel the token - just call StopAsync to trigger graceful deceleration
